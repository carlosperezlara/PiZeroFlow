...
void QC_Cumulants::Results() {
  ...
  for(int pt=0; pt!=fP2PC->GetXaxis()->GetNbins(); ++pt) { // as good as any
    tD2[pt]  = new TH1D( Form("D2_%d_%s_UNBINNED",pt,fList->GetName()), "D2;d\{2}_UNBINNED", nma-1,nmas);
    tD4[pt]  = new TH1D( Form("D4_%d_%s_UNBINNED",pt,fList->GetName()), "D4;d\{4}_UNBINNED", nma-1,nmas);
    tPV2[pt] = new TH1D( Form("PV2_%d_%s_UNBINNED",pt,fList->GetName()),"PV2;v\{2}_UNBINNED",nma-1,nmas);
    tPV4[pt] = new TH1D( Form("PV4_%d_%s_UNBINNED",pt,fList->GetName()),"PV4;v\{4}_UNBINNED",nma-1,nmas);
    tFD2[pt]  = new TH1D( Form("D2_%d_%s_BINNED",pt,fList->GetName()), "D2;d\{2}_BINNED", nma0-1,nmas0);
    tFD4[pt]  = new TH1D( Form("D4_%d_%s_BINNED",pt,fList->GetName()), "D4;d\{4}_BINNED", nma0-1,nmas0);
    tFPV2[pt] = new TH1D( Form("PV2_%d_%s_BINNED",pt,fList->GetName()),"PV2;v\{2}_BINNED",nma0-1,nmas0);
    tFPV4[pt] = new TH1D( Form("PV4_%d_%s_BINNED",pt,fList->GetName()),"PV4;v\{4}_BINNED",nma0-1,nmas0);
    fListResults->Add(tD2[pt]);
    fListResults->Add(tD4[pt]);
    fListResults->Add(tPV2[pt]);
    fListResults->Add(tPV4[pt]);
    fListResults->Add(tFD2[pt]);
    fListResults->Add(tFD4[pt]);
    fListResults->Add(tFPV2[pt]);
    fListResults->Add(tFPV4[pt]);
    //UNBINNED
  }

  //====
  for(Int_t binx=1; binx<fBinMap->GetXaxis()->GetNbins()+1; ++binx)
    for(Int_t biny=1; biny<fBinMap->GetYaxis()->GetNbins()+1; ++biny) {
