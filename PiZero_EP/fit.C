...
int fit(int ptbin=5, TString method="EP") {
  Double_t minX = 0.055;
  Double_t maxX = 0.240;
  gStyle->SetOptStat(0);
  ...
  //////////////////////////
  //FIT ALTERNATIVO
  TF1 *yieldF = new TF1("yieldF",Form("[0]*TMath::Gaus(x,[1],[2])+%s",pol7fix.Data()));
  ...
  */
  ...
  TF1 *flowBgrF = new TF1("flowBgrF", Form("([3]*%s+([4]+[5]*(x-[1])+[6]*pow(x-[1],2) )*(%s))/(%s+%s)",
					   gaus.Data(),pol7fix.Data(),
					   gaus.Data(),pol7fix.Data()),minX,maxX );
